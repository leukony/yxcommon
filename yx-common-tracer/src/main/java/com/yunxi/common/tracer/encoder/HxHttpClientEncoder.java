 ...
 * @version $Id: HxHttpClientEncoder.java, v 0.1 2017年1月11日 下午6:12:21 leukony Exp $
 */
public class HxHttpClientEncoder implements TracerEncoder<HttpContext> {
    ...
    public void encode(HttpContext ctx, TracerAppender appender) throws IOException {
        ...
        tb.appendRaw(PREFIX_z).appendRaw(DateUtils.format(ctx.getFinishTime(), DateUtils.MILLS_FORMAT_PATTERN)).appendRaw(SUFFIX_z).appendRaw(SPACE)
          ...
          .appendRaw("\"reqSize\":\"" + ctx.getRequestSize() + "\",")
          .appendRaw("\"resSize\":\"" + ctx.getResponseSize() + "\",")
